...
usage() {
  cat <<'EOF'
Usage:
  bash train_code.sh -method {grpo|dapo} -reward {ori|new|pdpo|gdpo} -model {qwen3-4b|qwen3-8b|deepseek7b|custom} [options]
...
Examples:
  bash train_code.sh -method grpo -reward ori -model qwen3-4b
...
case "${METHOD_KIND}" in
  ...
  dapo)
    ...
    METHOD_ARGS+=("actor_rollout_ref.actor.clip_ratio_low=${PPO_CLIP_RATIO_LOW}")
    METHOD_ARGS+=("actor_rollout_ref.actor.clip_ratio_high=${PPO_CLIP_RATIO_HIGH}")
...
# GDPO should be dominated by main pass-rate/accuracy. Keep extractability as a
# logged reward_extra metric by default, not an advantage input.
GDPO_ENABLE_CODING_CODE_EXTRACTABILITY_REWARD=${GDPO_ENABLE_CODING_CODE_EXTRACTABILITY_REWARD:-false}
GDPO_WEIGHT_CODING_CODE_EXTRACTABILITY_REWARD=${GDPO_WEIGHT_CODING_CODE_EXTRACTABILITY_REWARD:-0.0}
GDPO_ENABLE_CODING_SYNTAX_VALIDITY_REWARD=${GDPO_ENABLE_CODING_SYNTAX_VALIDITY_REWARD:-true}
GDPO_WEIGHT_CODING_SYNTAX_VALIDITY_REWARD=${GDPO_WEIGHT_CODING_SYNTAX_VALIDITY_REWARD:-0.05}
GDPO_ENABLE_CODING_UNIT_TEST_PASS_RATE=${GDPO_ENABLE_CODING_UNIT_TEST_PASS_RATE:-false}
GDPO_WEIGHT_CODING_UNIT_TEST_PASS_RATE=${GDPO_WEIGHT_CODING_UNIT_TEST_PASS_RATE:-0.0}
GDPO_ENABLE_CODING_COMPILER_RUNTIME_FEEDBACK=${GDPO_ENABLE_CODING_COMPILER_RUNTIME_FEEDBACK:-true}
GDPO_WEIGHT_CODING_COMPILER_RUNTIME_FEEDBACK=${GDPO_WEIGHT_CODING_COMPILER_RUNTIME_FEEDBACK:-0.05}
GDPO_ENABLE_CODING_STATIC_ANALYSIS_REWARD=${GDPO_ENABLE_CODING_STATIC_ANALYSIS_REWARD:-false}
GDPO_WEIGHT_CODING_STATIC_ANALYSIS_REWARD=${GDPO_WEIGHT_CODING_STATIC_ANALYSIS_REWARD:-0.0}
GDPO_ENABLE_CODING_EXECUTED_TOKEN_CREDIT=${GDPO_ENABLE_CODING_EXECUTED_TOKEN_CREDIT:-false}
GDPO_WEIGHT_CODING_EXECUTED_TOKEN_CREDIT=${GDPO_WEIGHT_CODING_EXECUTED_TOKEN_CREDIT:-0.0}
GDPO_ENABLE_CODING_BLOCK_LEVEL_PROCESS_REWARD=${GDPO_ENABLE_CODING_BLOCK_LEVEL_PROCESS_REWARD:-false}
GDPO_WEIGHT_CODING_BLOCK_LEVEL_PROCESS_REWARD=${GDPO_WEIGHT_CODING_BLOCK_LEVEL_PROCESS_REWARD:-0.0}
...
if [[ "${ADV_ESTIMATOR:-grpo}" == "gdpo" ]]; then
  ...
  if [[ -z "${GDPO_REWARD_KEYS:-}" ]]; then
    ...
    if is_truthy "${CODING_ENABLE_CODE_EXTRACTABILITY_REWARD}" && is_truthy "${GDPO_ENABLE_CODING_CODE_EXTRACTABILITY_REWARD}"; then
      gdpo_keys+=("coding_code_extractability_reward")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_CODE_EXTRACTABILITY_REWARD}")
    fi
    if is_truthy "${CODING_ENABLE_SYNTAX_VALIDITY_REWARD}" && is_truthy "${GDPO_ENABLE_CODING_SYNTAX_VALIDITY_REWARD}"; then
      gdpo_keys+=("coding_syntax_validity_reward")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_SYNTAX_VALIDITY_REWARD}")
    fi
    if is_truthy "${CODING_ENABLE_UNIT_TEST_PASS_RATE}" && is_truthy "${GDPO_ENABLE_CODING_UNIT_TEST_PASS_RATE}"; then
      gdpo_keys+=("coding_unit_test_pass_rate")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_UNIT_TEST_PASS_RATE}")
    fi
    if is_truthy "${CODING_ENABLE_COMPILER_RUNTIME_FEEDBACK}" && is_truthy "${GDPO_ENABLE_CODING_COMPILER_RUNTIME_FEEDBACK}"; then
      gdpo_keys+=("coding_compiler_runtime_feedback")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_COMPILER_RUNTIME_FEEDBACK}")
    fi
    if is_truthy "${CODING_ENABLE_STATIC_ANALYSIS_REWARD}" && is_truthy "${GDPO_ENABLE_CODING_STATIC_ANALYSIS_REWARD}"; then
      gdpo_keys+=("coding_static_analysis_reward")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_STATIC_ANALYSIS_REWARD}")
    fi
    if is_truthy "${CODING_ENABLE_EXECUTED_TOKEN_CREDIT}" && is_truthy "${GDPO_ENABLE_CODING_EXECUTED_TOKEN_CREDIT}"; then
      gdpo_keys+=("coding_executed_token_credit")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_EXECUTED_TOKEN_CREDIT}")
    fi
    if is_truthy "${CODING_ENABLE_BLOCK_LEVEL_PROCESS_REWARD}" && is_truthy "${GDPO_ENABLE_CODING_BLOCK_LEVEL_PROCESS_REWARD}"; then
      gdpo_keys+=("coding_block_level_process_reward")
      gdpo_weights+=("${GDPO_WEIGHT_CODING_BLOCK_LEVEL_PROCESS_REWARD}")
    fi
...
# 1) Experiment config
############################################
PROJECT_NAME=${PROJECT_NAME:-"eurus_${METHOD_LABEL}"}
if [[ -n "${RUN_TAG}" ]]; then
  DEFAULT_EXP_NAME="${METHOD_LABEL}-${MODEL_TAG}-${REWARD_LABEL}-${KL_LABEL}-${RUN_TAG}-${RUN_INSTANCE_TAG}"
else
  DEFAULT_EXP_NAME="${METHOD_LABEL}-${MODEL_TAG}-${REWARD_LABEL}-${KL_LABEL}-${RUN_INSTANCE_TAG}"
fi
...
PDPO_ANSWER_GATE_CHANNEL=${PDPO_ANSWER_GATE_CHANNEL:-coding_code_extractability_reward}
...
PDPO_FORMAT_CONSTRAINT_RELIABILITY_ENABLED=${PDPO_FORMAT_CONSTRAINT_RELIABILITY_ENABLED:-false}
...
PDPO_SAFETY_DUAL_ETA=${PDPO_SAFETY_DUAL_ETA:-0.02}
...
PDPO_NEED_DUAL_EMA_ALPHA=${PDPO_NEED_DUAL_EMA_ALPHA:-0.10}
...
CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES} python3 -m verl.trainer.main_ppo \
  ...
  ++reward_model.reward_kwargs.pdpo_format_constraint_channels="'${PDPO_FORMAT_CONSTRAINT_CHANNELS}'" \
  ++reward_model.reward_kwargs.pdpo_efficiency_cost_channels="'${PDPO_EFFICIENCY_COST_CHANNELS}'" \
  ++reward_model.reward_kwargs.pdpo_efficiency_cost_weight_cap="${PDPO_EFFICIENCY_COST_WEIGHT_CAP}" \
  ++reward_model.reward_kwargs.pdpo_format_constraint_reliability_enabled="${PDPO_FORMAT_CONSTRAINT_RELIABILITY_ENABLED}" \
  ++reward_model.reward_kwargs.pdpo_format_constraint_safety_enabled="${PDPO_FORMAT_CONSTRAINT_SAFETY_ENABLED}" \
  ...
  ++reward_model.reward_kwargs.pdpo_need_dual_enabled="${PDPO_NEED_DUAL_ENABLED}" \
  ++reward_model.reward_kwargs.pdpo_need_dual_eta="${PDPO_NEED_DUAL_ETA}" \
  ++reward_model.reward_kwargs.pdpo_need_dual_target="${PDPO_NEED_DUAL_TARGET}" \
  ++reward_model.reward_kwargs.pdpo_need_dual_max="${PDPO_NEED_DUAL_MAX}" \
  ++reward_model.reward_kwargs.pdpo_need_dual_ema_alpha="${PDPO_NEED_DUAL_EMA_ALPHA}" \
